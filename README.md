...
> ⚠️ 以下は現在進行中の思想であり、今後の開発方針として記録しています。現バージョンの機能とは必ずしも一致しません。

子供にお手伝いをさせるためのアプリ、という出発点から始まりましたが、開発を続けるうちに本質的な問いが生まれました。

**「子供の動機をコントロールすることは、親の傲慢ではないか？」**

今の子供たちの周りには、YouTubeやSNS、Robloxなど、承認欲求をくすぐる誘惑が絶え間なく存在します。それらは巧妙に設計されており、子供の注意と時間を奪い続けます。親が何もしなければ、子供の承認欲求はスクリーンの中で消費されていきます。

このアプリが本当に解きたい問題は、**「子供の承認欲求を、家族の中で満たせる仕組みを作る」** ことです。

お手伝いの申請・承認フローは、単なる管理の仕組みではありません。子供が行動し、親がそれを見て認める——この一連のやりとりは、「自分は見られている、認められている」という感覚を毎回届ける装置として機能します。YouTubeの「いいね」に対して、家族の「承認」で応えられる仕組みです。

目指すのは、子供の将来を外部のプラットフォームに委ねず、**家族の関係性の中で承認欲求を健全に育てられる環境**を、日常の小さな習慣から作ること。そのための道具としてこのアプリを育てていきたいと考えています。
