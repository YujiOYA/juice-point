...
export function useManagerPanel(submissions: Submission[]) {
    ...
    const pending = submissions
        .filter((s) => s.status === SUBMISSION_STATUS.pending && s.submissionType !== SubmissionType.TaskRequest)
        .sort(byDateDesc);
    const rejected = submissions
        .filter((s) => s.status === SUBMISSION_STATUS.rejected && s.submissionType !== SubmissionType.TaskRequest)
        .sort(byDateDesc);
    const pendingTaskRequests = submissions
        .filter((s) => s.status === SUBMISSION_STATUS.pending && s.submissionType === SubmissionType.TaskRequest)
        .sort(byDateDesc);
    const rejectedTaskRequests = submissions
        .filter((s) => s.status === SUBMISSION_STATUS.rejected && s.submissionType === SubmissionType.TaskRequest)
        .sort(byDateDesc);
