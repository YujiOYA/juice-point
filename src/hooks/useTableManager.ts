...
interface Options<Item extends { id: string; whose: string }, SK extends string, F extends Record<string, string>> {
    items: Item[];
...
>(opts: Options<Item, SK, F>) {
    ...
    const sortedItems = useMemo(() => {
        return [...items]
            ...
            .sort((a, b) => {
                const av = sortKeyExtractor(a, sortKey),
                    bv = sortKeyExtractor(b, sortKey);
                if (av !== bv) return (av < bv ? -1 : 1) * (sortDir === "asc" ? 1 : -1);
                if (!sortKey2) return 0;
                const av2 = sortKeyExtractor(a, sortKey2),
                    bv2 = sortKeyExtractor(b, sortKey2);
