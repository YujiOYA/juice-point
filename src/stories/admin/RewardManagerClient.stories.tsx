...
- ミューテーション（追加・更新・削除）成功後、\`invalidateQueries\` により自動再フェッチされる
  ...
  argTypes: {
    users: {
      description: "**サーバー取得**｜`admin/page.tsx` が DynamoDB から取得。担当者の選択肢として使用。TanStack Query は使用しない。",
    },
    initialRewards: {
      description: "**サーバー取得 → TanStack Query**｜`admin/page.tsx` が DynamoDB から取得し、`useRewards(initialRewards)` の `initialData` として渡す。ミューテーション後に自動再フェッチ。",
    },
  },
};
