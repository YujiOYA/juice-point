...
- ログイン処理（PIN照合・セッション発行）は API への fetch を直接行う。TanStack Query は使用しない。
  ...
  argTypes: {
    users: {
      description: "**サーバー取得**｜`page.tsx` が DynamoDB から取得。ログインユーザーの選択肢として表示。TanStack Query は使用しない。",
    },
    submissions: {
      description: "**サーバー取得**｜`page.tsx` が DynamoDB から取得。ログイン中ユーザーの未使用ポイント計算に使用。TanStack Query は使用しない。",
    },
    loggedInUser: {
      description: "**クライアント状態**｜`PageClient` の `useState` が管理。`null` のときログインフォームを表示。",
    },
    setLoggedInUser: {
      description: "**クライアント状態セッター**｜ログイン成功・ログアウト時に `PageClient` の state を更新する。",
    },
    next: {
