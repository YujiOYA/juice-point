...
- 内部の \`useTaskForm\` がすべて **TanStack Query** に流し込む
  ...
  - \`initialRewards\` → \`useRewards(initialRewards)\` の \`initialData\`
    ...
    argTypes: {
        ...
        initialTasks: {
            description:
                "**サーバー取得 → TanStack Query**｜`page.tsx` が DynamoDB から取得し、`useTasks(initialTasks)` の `initialData` として渡す。",
        },
        initialSubmissions: {
            description:
                "**サーバー取得 → TanStack Query**｜`page.tsx` が DynamoDB から取得し、`useSubmissions(initialSubmissions)` の `initialData` として渡す。申請操作後に自動再フェッチされる。",
        },
        initialRewards: {
            description:
                "**サーバー取得 → TanStack Query**｜`page.tsx` が DynamoDB から取得し、`useRewards(initialRewards)` の `initialData` として渡す。",
        },
    },
};
