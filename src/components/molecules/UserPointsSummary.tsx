...
export default function UserPointsSummary({ users, submissions }: Props) {
  ...
  const BreakdownItem = ({ s }: { s: Submission }) => {
    ...
    return (
      <li style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", fontSize: "0.85rem", color: "#555", padding: "0.3rem 0 0.3rem 0.75rem", borderLeft: "2px solid #ffe0b2" }}>
        {isEditing ? (
          <>
            ...
            <input
              ...
              step={0.5}
              style={{ width: "4.5rem", padding: "0.1rem 0.3rem", border: "1px solid #d1d5db", borderRadius: "4px", fontSize: "0.85rem" }}
            />
            <Button variant="approve" disabled={updatePoint.isPending} onClick={() => handleSave(s.id)} style={{ padding: "0.1rem 0.4rem", fontSize: "0.7rem", flex: "none", width: "auto" }}>保存</Button>
            <Button variant="logout" onClick={() => setEditingId(null)} style={{ padding: "0.1rem 0.4rem", fontSize: "0.7rem", flex: "none" }}>キャンセル</Button>
          </>
        ) : (
          <>
            <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.whatYouDid}</span>
  ...
  const BreakdownList = ({ approved }: { approved: Submission[] }) => (
    <ul style={{ listStyle: "none", padding: "0.5rem 0 0", margin: 0, display: "flex", flexDirection: "column", gap: "0.25rem" }}>
      {approved.map((s) => <BreakdownItem key={s.id} s={s} />)}
    </ul>
  );
  ...
  return (
    <div>
          ...
          <div key={user.id} className="submission-card">
            ...
            >
              <span style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontWeight: 600 }}>
                <span style={{ display: "inline-block", transition: "transform 0.2s", transform: openUserId === user.id ? "rotate(90deg)" : "rotate(0deg)", color: "#9e9e9e" }}>▶</span>
                {user.user}
              </span>
      ...
      <table className="manager-table">
        <thead>
          <tr>
            <th><SortBtn k="user" label="担当者" /></th>
            <th><SortBtn k="point" label="未使用ポイント" /></th>
            <th>内訳</th>
          </tr>
        </thead>
        <tbody>
